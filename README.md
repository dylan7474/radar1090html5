...
- Automatic alert detection surfaces rapid descents, critical squawk codes, and inbound traffic with prominent warnings in the data panel, highlights the affected blip on the scope, and names the flight in a single-line scrolling alert banner that queues multiple alerts and cycles through each one until the underlying condition clears—no more mid-scroll resets.

...
- Automatic alert detection surfaces rapid descents, critical squawk codes, and inbound traffic with prominent warnings in the data panel and highlights the affected blip on the scope. Receiver and server warnings share a scrolling message banner that queues every alert, lets each one glide fully across the display, and never clears a ticker mid-scroll even when new notices arrive.
